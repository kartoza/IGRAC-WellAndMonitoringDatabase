...
from gwml2.models import (
    TermMeasurementParameter, MEASUREMENT_PARAMETER_AMSL, Well, Unit,
...
class NetherlandHarvester(BaseHarvester):
    ...
    def __init__(
        ...
        super(NetherlandHarvester, self).__init__(
    ...
    def fetch_stations(self, url):
        ...
        for feature in data['features']:
            try:
                ...
                updated = self.process_level_measurement(harvester_well_data)
    ...
    def process_level_measurement(
        ...
        last_measurement = well.welllevelmeasurement_set.order_by(
            '-time').first()
        for row in rows:
            if not row['Tijdstip'] or not row['Beoordeelde Waarde [m]']:
                continue
