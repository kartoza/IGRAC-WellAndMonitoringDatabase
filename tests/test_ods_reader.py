...
class ODSReaderTest(GWML2Test):
    ...
    def test_count(self):
        """To file exist."""
        file_path = absolute_path("gwml2", "tests", "fixtures", "wells.old.ods")
    ...
    def test_sheet_not_found(self):
        """To file exist."""
        file_path = absolute_path("gwml2", "tests", "fixtures", "monitoring_data.ods")
        upload_session = UploadSession.objects.create()
        with self.assertRaises(KeyError) as e:
            GeneralInformationUploader(upload_session, {}, 0, 1, file_path=file_path)

        HydrogeologyUploader(upload_session, {}, 0, 1, file_path=file_path)
        ManagementUploader(upload_session, {}, 0, 1, file_path=file_path)
        DrillingAndConstructionUploader(upload_session, {}, 0, 1, file_path=file_path)
        WaterStrikeUploader(upload_session, {}, 0, 1, file_path=file_path)
        StratigraphicLogUploader(upload_session, {}, 0, 1, file_path=file_path)
        StructuresUploader(upload_session, {}, 0, 1, file_path=file_path)
        file_path = absolute_path("gwml2", "tests", "fixtures", "wells.ods")

        with self.assertRaises(KeyError):
            MonitoringDataUploader(upload_session, {}, 0, 1, file_path=file_path)
    ...
    def check_old_file(self, Uploader, file_name):
        ...
        with self.assertRaises(ExcelOutOfDate) as e:
            Uploader(
                ...
                0,
                1,
                file_path=absolute_path("gwml2", "tests", "fixtures", file_name),
            )
            self.assertEquals(
                f"{e}", "The file is out of date, please download the latest template on the form"
            )
    ...
    def test_sheet_old_version(self):
        ...
        self.check_old_file(DrillingAndConstructionUploader, "drilling_and_construction.old.ods")
        self.check_old_file(WaterStrikeUploader, "drilling_and_construction.old.ods")
        self.check_old_file(StratigraphicLogUploader, "drilling_and_construction.old.ods")
        self.check_old_file(StructuresUploader, "drilling_and_construction.old.ods")
        self.check_old_file(MonitoringDataUploader, "monitoring_data.old.ods")
    ...
    @patch.object(GeneralInformationUploader, "convert_record", new=convert_record)
    ...
    def test_well_file(self):
        ...
        # General information
        GeneralInformationUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["General Information"][0], captured_data["General Information"][0])
        self.compare(data["General Information"][1], captured_data["General Information"][1])

        # Hydrogeology
        HydrogeologyUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Hydrogeology"][0], captured_data["Hydrogeology"][0])
        self.compare(data["Hydrogeology"][1], captured_data["Hydrogeology"][1])

        # Management
        ManagementUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Management"][0], captured_data["Management"][0])
        self.compare(data["Management"][1], captured_data["Management"][1])

    @patch.object(DrillingAndConstructionUploader, "convert_record", new=convert_record)
    @patch.object(WaterStrikeUploader, "convert_record", new=convert_record)
    @patch.object(StratigraphicLogUploader, "convert_record", new=convert_record)
    @patch.object(StructuresUploader, "convert_record", new=convert_record)
    def test_drilling_and_construction_file(self):
        ...
        file_path = absolute_path("gwml2", "tests", "fixtures", "drilling_and_construction.ods")
        # drilling and construction
        DrillingAndConstructionUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Drilling"][0], captured_data["Drilling"][0])
        self.compare(data["Drilling"][1], captured_data["Drilling"][1])
        # Water strike
        WaterStrikeUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Water Strike"][0], captured_data["Water Strike"][0])
        self.compare(data["Water Strike"][1], captured_data["Water Strike"][1])
        # Stratigraphic Log
        StratigraphicLogUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Stratigraphic Log"][0], captured_data["Stratigraphic Log"][0])
        self.compare(data["Stratigraphic Log"][1], captured_data["Stratigraphic Log"][1])
        # Construction
        StructuresUploader(UploadSession.objects.create(), 0, 1, file_path=file_path)
        self.compare(data["Construction"][0], captured_data["Construction"][0])
        self.compare(data["Construction"][1], captured_data["Construction"][1])
    ...
    def test_resumed(self):
        ...
        file_path = absolute_path("gwml2", "tests", "fixtures", "wells.ods")
        status = {
            "General Information": {"added": 1, "error": 0, "skipped": 0},
            "Hydrogeology": {"added": 1, "error": 0, "skipped": 0},
            "Management": {"added": 1, "error": 0, "skipped": 0},
        }
        GeneralInformationUploader(
            UploadSession.objects.create(status=json.dumps(status)), 0, 1, file_path=file_path
        )
        HydrogeologyUploader(
            UploadSession.objects.create(status=json.dumps(status)), 0, 1, file_path=file_path
        )
        ...
        self.compare(data["General Information"][1], captured_data["General Information"][0])
        self.compare(data["Hydrogeology"][1], captured_data["Hydrogeology"][0])
        self.compare(data["Management"][1], captured_data["Management"][0])
