...
class OrganisationTest(GWML2Test):
    ...
    @patch(
        'gwml2.models.well_management.organisation.Organisation.update_ggis_uid_background'
    )
    def test_change_ggis_uid_being_called(
            self, mock_update_ggis_uid_background, mock_update_ggis_uid
    ):
        ...
        self.assertEqual(mock_update_ggis_uid_background.call_count, 0)
        organisation.name = 'Organisation B'
        organisation.save()
        self.assertEqual(mock_update_ggis_uid.call_count, 0)
        self.assertEqual(mock_update_ggis_uid_background.call_count, 1)
        ...
        organisation.save()
        self.assertEqual(mock_update_ggis_uid.call_count, 0)
        self.assertEqual(mock_update_ggis_uid_background.call_count, 2)

    # TODO
    #  We will fix this as it is on the background
