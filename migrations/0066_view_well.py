...
class Migration(migrations.Migration):
    dependencies = [
        ('gwml2', '0065_organisation_active'),
    ]
    ...
    sql = """
            ...
            CREATE MATERIALIZED VIEW mv_well_ggmn AS select DISTINCT ON (id) id, organisation || '-' || original_id AS ggis_uid, original_id, name, feature_type,purpose, status,organisation, country, year_of_drilling, aquifer_name, aquifer_type,manager, detail, location, created_at, created_by, last_edited_at, last_edited_by from vw_well where number_of_measurements > 0 and organisation is not null;
            CREATE MATERIALIZED VIEW mv_well AS select DISTINCT ON (id) id, organisation || '-' || original_id AS ggis_uid, original_id, name, feature_type,purpose, status,organisation, country, year_of_drilling, aquifer_name, aquifer_type,manager, detail, location, created_at, created_by, last_edited_at, last_edited_by from vw_well;
        """
